...
describe("Functions", function () {
  describe("roundTo()", function () {
    ...
    it("should handle roundTo(-23.67, 1)", function () {
      assert.strictEqual(Parser.evaluate("roundTo(-23.67, 3)"), -23.67);
    });

    // Big numbers
    it("should handle roundTo(3000000000000000000000000.1233, 1)", function () {
      assert.strictEqual(
        Parser.evaluate("roundTo(3000000000000000000000000.1233, 1)"),
        // eslint-disable-next-line no-loss-of-precision
        3000000000000000000000000.1,
      );
    });
    it("should handle roundTo(-3000000000000000000000000.1233, 1)", function () {
      assert.strictEqual(
  ...
  describe("fac(n)", function () {
    it("should return n!", function () {
      ...
      assert.ok(
        Math.abs(parser.evaluate("fac(2.5)") - 3.323350970447843) <= 1e-14,
      );
      assert.strictEqual(parser.evaluate("fac(3)"), 6);
      assert.strictEqual(parser.evaluate("fac(4)"), 24);
      assert.ok(
        Math.abs(parser.evaluate("fac(4.9)") - 101.27019121310335) <= 1e-12,
      );
  ...
  describe("hypot(a, b, ...)", function () {
    ...
    it("should avoid overflowing", function () {
      assert.ok(
        Math.abs(
          parser.evaluate("hypot(10^200, 10^200)") - 1.4142135623730959e200,
        ) <= 1e186,
      );
      assert.ok(
        Math.abs(
          parser.evaluate("hypot(10^-200, 10^-200)") - 1.4142135623730944e-200,
        ) <= 1e186,
      );
      assert.ok(
        Math.abs(
          parser.evaluate("hypot(10^100, 11^100, 12^100, 13^100)") -
            2.4793352492856554e111,
        ) <= 1e97,
      );
      assert.strictEqual(
        parser.evaluate("hypot(x)", { x: Number.MAX_VALUE }),
        Number.MAX_VALUE,
      );
      assert.strictEqual(
        parser.evaluate("hypot(x, 0)", { x: Number.MAX_VALUE }),
        Number.MAX_VALUE,
      );
    });
  });
  ...
  describe("atan2(y, x)", function () {
    it("should return atan(y / x)", function () {
      ...
      assert.strictEqual(
        parser.evaluate("atan2(0, 1/-inf)", { inf: Infinity }),
        Math.PI,
      );
      assert.strictEqual(
        parser.evaluate("atan2(1/-inf, 1/-inf)", { inf: Infinity }),
        -Math.PI,
      );
    });
  });
  ...
  describe("if(p, t, f)", function () {
    ...
    it("if(3 && 6, if(45 > 5 * 11, 3 * 3, 2.4), 0)", function () {
      assert.strictEqual(
        Parser.evaluate("if(3 and 6, if(45 > 5 * 11, 3 * 3, 2.4), 0)"),
        2.4,
      );
    });
  });
  ...
  describe("gamma(x)", function () {
    ...
    it("returns exact answer for integers", function () {
      ...
      assert.strictEqual(parser.evaluate("gamma(25)"), 6.204484017332394e23);
      assert.strictEqual(parser.evaluate("gamma(50)"), 6.082818640342679e62);
      assert.strictEqual(parser.evaluate("gamma(100)"), 9.332621544394415e155);
      assert.strictEqual(parser.evaluate("gamma(170)"), 4.2690680090047056e304);
      assert.strictEqual(parser.evaluate("gamma(171)"), 7.257415615308004e306);
    ...
    it("returns approximation for fractions", function () {
      ...
      assert.ok(
        Math.abs(parser.evaluate("gamma(-2.5)") - -0.9453087204829419) <= delta,
      );
      assert.strictEqual(parser.evaluate("gamma(-2)"), Infinity);
      assert.ok(
        Math.abs(parser.evaluate("gamma(-1.5)") - 2.36327180120735) <= delta,
      );
      assert.strictEqual(parser.evaluate("gamma(-1)"), Infinity);
      assert.ok(
        Math.abs(parser.evaluate("gamma(-0.75)") - -4.834146544295878) <= delta,
      );
      assert.ok(
        Math.abs(parser.evaluate("gamma(-0.5)") - -3.54490770181103) <= delta,
      );
      assert.ok(
        Math.abs(parser.evaluate("gamma(-0.25)") - -4.901666809860711) <= delta,
      );
      assert.ok(
        Math.abs(parser.evaluate("gamma(0.25)") - 3.62560990822191) <= delta,
      );
      assert.ok(
        Math.abs(parser.evaluate("gamma(0.5)") - 1.77245385090552) <= delta,
      );
      assert.ok(
        Math.abs(parser.evaluate("gamma(0.75)") - 1.22541670246518) <= delta,
      );
      assert.ok(
        Math.abs(parser.evaluate("gamma(1.5)") - 0.886226925452758) <= delta,
      );
      assert.ok(
        Math.abs(parser.evaluate("gamma(84.9)") - 2.12678581783475e126) <=
          1e113,
      );
      assert.ok(
        Math.abs(parser.evaluate("gamma(85.1)") - 5.16530039995559e126) <=
          1e114,
      );
      assert.ok(
        Math.abs(parser.evaluate("gamma(98.6)") - 1.5043414559976e153) <= 1e140,
      );
  ...
  describe("map(f, a)", function () {
    ...
    it("should call built-in functions", function () {
      ...
      assert.deepStrictEqual(
        parser.evaluate("map(sqrt, [0, 1, 16, 81])"),
        [0, 1, 4, 9],
      );
      assert.deepStrictEqual(
        parser.evaluate("map(max, [2, 2, 2, 2, 2, 2])"),
        [2, 2, 2, 3, 4, 5],
      );
    });
    ...
    it("should call self-defined functions", function () {
      var parser = new Parser();
      assert.deepStrictEqual(
        parser.evaluate("f(a) = a*a; map(f, [0, 1, 2, 3, 4])"),
        [0, 1, 4, 9, 16],
      );
    });
    ...
    it("should call self-defined functions with index", function () {
      var parser = new Parser();
      assert.deepStrictEqual(
        parser.evaluate("f(a, i) = a+i; map(f, [1,3,5,7,9])"),
        [1, 4, 7, 10, 13],
      );
      assert.deepStrictEqual(
        parser.evaluate("map(anon(a, i) = a+i, [1,3,5,7,9])"),
        [1, 4, 7, 10, 13],
      );
      assert.deepStrictEqual(
        parser.evaluate("f(a, i) = i; map(f, [1,3,5,7,9])"),
        [0, 1, 2, 3, 4],
      );
    });
  });
  ...
  describe("fold(f, init, array)", function () {
    ...
    it("should fail if third argument is not an array", function () {
      var parser = new Parser();
      assert.throws(function () {
        parser.evaluate("fold(random, 0, 5)");
      }, /not an array/);
    });
    ...
    it("should call built-in functions", function () {
      var parser = new Parser();
      assert.strictEqual(
        parser.evaluate("fold(max, -1, [1, 3, 5, 4, 2, 0])"),
        5,
      );
      assert.strictEqual(
        parser.evaluate("fold(min, 10, [1, 3, 5, 4, 2, 0, -2, -1])"),
        -2,
      );
    });
    ...
    it("should call self-defined functions", function () {
      var parser = new Parser();
      assert.strictEqual(
        parser.evaluate("f(a, b) = a*b; fold(f, 1, [1, 2, 3, 4, 5])"),
        120,
      );
    });
    ...
    it("should call self-defined functions with index", function () {
      var parser = new Parser();
      assert.strictEqual(
        parser.evaluate("f(a, b, i) = a*i + b; fold(f, 100, [1,3,5,7,9])"),
        193,
      );
    });
    ...
    it("should start with the accumulator", function () {
      var parser = new Parser();
      assert.strictEqual(
        parser.evaluate("f(a, b) = a*b; fold(f, 0, [1, 2, 3, 4, 5])"),
        0,
      );
      assert.strictEqual(
        parser.evaluate("f(a, b) = a*b; fold(f, 1, [1, 2, 3, 4, 5])"),
      ...
      assert.strictEqual(
        parser.evaluate("f(a, b) = a*b; fold(f, 2, [1, 2, 3, 4, 5])"),
        240,
      );
      assert.strictEqual(
        parser.evaluate("f(a, b) = a*b; fold(f, 3, [1, 2, 3, 4, 5])"),
        360,
      );
    });
  });
  ...
  describe("filter(f, array)", function () {
    ...
    it("should fail if second argument is not an array", function () {
      var parser = new Parser();
      assert.throws(function () {
        parser.evaluate("filter(random, 5)");
      }, /not an array/);
    });
    ...
    it("should call built-in functions", function () {
      var parser = new Parser();
      assert.deepStrictEqual(
        parser.evaluate('filter(not, [1, 0, false, true, 2, ""])'),
        [0, false, ""],
      );
    });
    ...
    it("should call self-defined functions", function () {
      var parser = new Parser();
      assert.deepStrictEqual(
        parser.evaluate("f(x) = x > 2; filter(f, [1, 2, 0, 3, -1, 4])"),
        [3, 4],
      );
      assert.deepStrictEqual(
        parser.evaluate("f(x) = x > 2; filter(f, [1, 2, 0, 1.9, -1, -4])"),
        [],
      );
    });
    ...
    it("should call self-defined functions with index", function () {
      var parser = new Parser();
      assert.deepStrictEqual(
        parser.evaluate("f(a, i) = a <= i; filter(f, [1,0,5,3,2])"),
        [0, 3, 2],
      );
      assert.deepStrictEqual(
        parser.evaluate("f(a, i) = i > 3; filter(f, [9,0,5,6,1,2,3,4])"),
        [1, 2, 3, 4],
      );
    });
  });
  ...
  describe("indexOf(target, string)", function () {
    ...
    it("should find the entire string", function () {
      var parser = new Parser();
      assert.strictEqual(
        parser.evaluate('indexOf("abcabcabc", "abcabcabc")'),
        0,
      );
    });
  ...
  describe("join(sep, array)", function () {
    ...
    it("should work on a multi-element arrays", function () {
      var parser = new Parser();
      assert.strictEqual(
        parser.evaluate('join("x", ["a", "b", "c", 4])'),
        "axbxcx4",
      );
