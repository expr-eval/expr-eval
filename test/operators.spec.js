...
describe("Operators", function () {
  ...
  describe("and operator", function () {
    ...
    it("skips rhs when lhs is false", function () {
      ...
      assert.strictEqual(
        Parser.evaluate("false and notCalled()", { notCalled: notCalled }),
        false,
      );
    ...
    it("evaluates rhs when lhs is true", function () {
      ...
      assert.strictEqual(
        parser.evaluate("true and spies.called()", {
          spies: { called: called },
        }),
        false,
      );
  ...
  describe("or operator", function () {
    ...
    it("skips rhs when lhs is true", function () {
      ...
      assert.strictEqual(
        Parser.evaluate("true or notCalled()", { notCalled: notCalled }),
        true,
      );
      assert.strictEqual(notCalled.called, false);
    });
    ...
    it("evaluates rhs when lhs is false", function () {
      ...
      parser.functions = { f: called };
      assert.strictEqual(
        parser.evaluate("false or spies.called()", {
          spies: { called: called },
        }),
        true,
      );
  ...
  describe("in operator", function () {
    ...
    it('"a" in ["a", "b"]', function () {
      assert.strictEqual(
        parser.evaluate('"a" in toto', { toto: ["a", "b"] }),
        true,
      );
    });
    ...
    it('"a" in ["b", "a"]', function () {
      assert.strictEqual(
        parser.evaluate('"a" in toto', { toto: ["b", "a"] }),
        true,
      );
    });
    ...
    it('"c" in ["a", "b"]', function () {
      assert.strictEqual(
        parser.evaluate('"c" in toto', { toto: ["a", "b"] }),
        false,
      );
    });
    ...
    it('"c" in ["b", "a"]', function () {
      assert.strictEqual(
        parser.evaluate('"c" in toto', { toto: ["b", "a"] }),
        false,
      );
    });
  ...
  describe("conditional operator", function () {
    ...
    it("a == 1 ? b == 2 ? 3 : 4 : 5", function () {
      assert.strictEqual(
        parser.evaluate("a == 1 ? b == 2 ? 3 : 4 : 5", { a: 1, b: 2 }),
        3,
      );
      assert.strictEqual(
        parser.evaluate("a == 1 ? b == 2 ? 3 : 4 : 5", { a: 1, b: 9 }),
        4,
      );
      assert.strictEqual(
        parser.evaluate("a == 1 ? b == 2 ? 3 : 4 : 5", { a: 9, b: 2 }),
        5,
      );
      assert.strictEqual(
        parser.evaluate("a == 1 ? b == 2 ? 3 : 4 : 5", { a: 9, b: 9 }),
        5,
      );
    });
  ...
  describe("sinh(x)", function () {
    it("returns the correct value", function () {
      ...
      assertCloseTo(
        parser.evaluate("sinh(2*PI)"),
        267.74489404101644,
        delta * 1000,
      );
      assertCloseTo(
        parser.evaluate("sinh -PI"),
        -11.548739357257748,
        delta * 10,
      );
      assertCloseTo(
        parser.evaluate("sinh(3*PI/2)"),
        55.65439759941754,
        delta * 100,
      );
      assertCloseTo(
        parser.evaluate("sinh 15"),
        1634508.6862359024,
        delta * 1000000,
      );
  ...
  describe("cosh(x)", function () {
    it("returns the correct value", function () {
      ...
      assertCloseTo(
        parser.evaluate("cosh(2*PI)"),
        267.7467614837483,
        delta * 1000,
      );
      assertCloseTo(
        parser.evaluate("cosh -PI"),
        11.591953275521522,
        delta * 10,
      );
      assertCloseTo(
        parser.evaluate("cosh(3*PI/2)"),
        55.663380890438695,
        delta * 100,
      );
      assertCloseTo(
        parser.evaluate("cosh 15"),
        1634508.6862362078,
        delta * 1e7,
      );
  ...
  describe("tanh(x)", function () {
    it("returns the correct value", function () {
      ...
      assertCloseTo(
        parser.evaluate("tanh 0.00001"),
        0.000009999999999621023,
        delta,
      );
  ...
  describe("x!", function () {
    ...
    it("returns exact answer for integers", function () {
      ...
      assert.strictEqual(parser.evaluate("25!"), 1.5511210043330984e25);
      assert.strictEqual(parser.evaluate("50!"), 3.0414093201713376e64);
      assert.strictEqual(parser.evaluate("100!"), 9.332621544394418e157);
      assert.strictEqual(parser.evaluate("170!"), 7.257415615308004e306);
  ...
  describe("[] operator", function () {
    ...
    it('a["foo"]', function () {
      assert.strictEqual(
        Parser.evaluate('a["foo"]', { a: { foo: "bar" } }),
        undefined,
      );
    });
  ...
  describe("sign(x)", function () {
    it("returns the sign of x", function () {
      ...
      assert.strictEqual(
        parser.parse("sign -0.001").simplify().toString(),
        "(-1)",
      );
  ...
  describe("expm1(x)", function () {
    it("returns e^x - 1", function () {
      ...
      assert.ok(
        /^1.718281828459\d*$/.test(
          parser.parse("expm1 1").simplify().toString(),
        ),
      );

      assertCloseTo(
        parser.parse("expm1 x").toJSFunction("x")(1),
        1.718281828459045,
        delta,
      );
      assertCloseTo(
        parser.parse("expm1 x").toJSFunction("x")(2),
        6.38905609893065,
        delta,
      );
    });
  ...
  describe("log1p(x)", function () {
    it("returns e^x - 1", function () {
      ...
      assertCloseTo(
        parser.parse("log1p x").toJSFunction("x")(1),
        0.6931471805599453,
        delta,
      );
      assertCloseTo(
        parser.parse("log1p x").toJSFunction("x")(9),
        2.302585092994046,
        delta,
      );
    });
  ...
  describe("log2(x)", function () {
    it("returns the base 2 log of x", function () {
      ...
      assertCloseTo(
        parser.parse("log2 x").toJSFunction("x")(3),
        1.584962500721156,
        delta,
      );
    });
