...
describe("Parser", function () {
  [
    ...
    {
      name: "disallowing member access",
      parser: new Parser({ allowMemberAccess: false }),
    },
    ...
    describe(tcase.name, function () {
      it("should skip comments", function () {
        assert.strictEqual(
          parser.evaluate("2/* comment */+/* another comment */3"),
          5,
        );
        assert.strictEqual(
          parser.evaluate("2/* comment *///* another comment */3"),
        ...
        assert.strictEqual(
          parser.evaluate(
            "/* comment at the beginning */2 + 3/* unterminated comment",
          ),
          5,
        );
        assert.strictEqual(
          parser.evaluate("2 +/* comment\n with\n multiple\n lines */3"),
          5,
        );
      });
      ...
      it("should accept variables starting with E", function () {
        assert.strictEqual(
          parser.parse("2 * ERGONOMIC").evaluate({ ERGONOMIC: 1000 }),
          2000,
        );
      });
      ...
      it("should accept variables starting with PI", function () {
        assert.strictEqual(
          parser.parse("1 / PITTSBURGH").evaluate({ PITTSBURGH: 2 }),
          0.5,
        );
      });
      ...
      it("should fail on invalid numbers", function () {
        ...
        assert.throws(function () {
          parser.parse("1.23ee4");
        }, Error);
        assert.throws(function () {
          parser.parse("1.23ee.4");
        }, Error);
        assert.throws(function () {
          parser.parse("1.23e4.0");
        }, Error);
        assert.throws(function () {
          parser.parse("123e.4");
        }, Error);
      ...
      it("should parse hexadecimal integers correctly", function () {
        ...
        assert.strictEqual(
          parser.evaluate("0x1FFFFFFFFFFFFF"),
          Math.pow(2, 53) - 1,
        );
        assert.strictEqual(
          parser.evaluate("0x20000000000000"),
          Math.pow(2, 53),
        );
      });
      ...
      it("should parse binary integers correctly", function () {
        ...
        assert.strictEqual(
          parser.evaluate("0b11111111111111111111111111111111"),
          Math.pow(2, 32) - 1,
        );
        assert.strictEqual(
          parser.evaluate("0b100000000000000000000000000000000"),
          Math.pow(2, 32),
        );
        assert.strictEqual(
          parser.evaluate(
      ...
      it("should parse strings", function () {
        ...
        assert.strictEqual(
          parser.evaluate("\"Nested 'single quotes'\""),
          "Nested 'single quotes'",
        );
        assert.strictEqual(
          parser.evaluate("'Nested \"double quotes\"'"),
          'Nested "double quotes"',
        );
        assert.strictEqual(
          parser.evaluate("'Single quotes \\'inside\\' single quotes'"),
          "Single quotes 'inside' single quotes",
        );
        assert.strictEqual(
          parser.evaluate('"Double quotes \\"inside\\" double quotes"'),
      ...
      it("should parse arrays correctly", function () {
        assert.strictEqual(
          parser.parse("[1, 2, 3+4, 5*6, (7/8)]").toString(),
          "[1, 2, (3 + 4), (5 * 6), (7 / 8)]",
        );
      });
      ...
      it("should parse operators that look like functions as function calls", function () {
        ...
        assert.strictEqual(
          parser.parse("sin(2)^3").toString(),
          "((sin 2) ^ 3)",
        );
        assert.strictEqual(
          parser.parse("sin 2^3").evaluate(),
          Math.sin(Math.pow(2, 3)),
        );
        assert.strictEqual(
          parser.parse("sin(2)^3").evaluate(),
          Math.pow(Math.sin(2), 3),
        );
      });
      ...
      it("should parse named prefix operators as function names at the end of expressions", function () {
        ...
        assert.deepStrictEqual(
          (function () {
            // @ts-expect-error
            parser.functions.f = function (a, b) {
              return [a, b];
            };
            return parser.parse("f(sin, sqrt)").evaluate();
          })(),
          [Math.sin, Math.sqrt],
        );
      ...
      it("should parse variables that start with operators", function () {
        ...
        assert.strictEqual(
          parser.parse("android * 2").toString(),
          "(android * 2)",
        );
        assert.strictEqual(
          parser.parse("single == 1").toString(),
          "(single == 1)",
        );
      });
      ...
      it("should parse valid variable names correctly", function () {
        ...
        assert.deepStrictEqual(parser.parse("A_C").variables(), ["A_C"]);
        assert.deepStrictEqual(
          parser.parse("abcdefg/hijklmnop+qrstuvwxyz").variables(),
        ...
        assert.deepStrictEqual(
          parser.parse("abc123+def456*ghi789/jkl0").variables(),
        ...
        assert.deepStrictEqual(parser.parse("$xyz_123").variables(), [
          "$xyz_123",
        ]);
        assert.deepStrictEqual(parser.parse("_xyz_123").variables(), [
          "_xyz_123",
        ]);
      });
      ...
      it("should allow operators to be explicitly enabled", function () {
        ...
        assert.strictEqual(
          parser.evaluate("3 in array", { array: [1, 2, 3] }),
          true,
        );
    ...
    it("should allow logical operators to be disabled", function () {
      ...
      assert.strictEqual(
        parser.evaluate("and * or + not", { and: 3, or: 5, not: 2 }),
        17,
      );
    });
  ...
  it("should disallow member access", function () {
    ...
    assert.throws(function () {
      parser.evaluate("min.bind");
    }, /member access is not permitted/);
    assert.throws(function () {
      parser.evaluate("min.bind()");
    }, /member access is not permitted/);
    assert.throws(function () {
      parser.evaluate("32 + min.bind");
    }, /member access is not permitted/);
    assert.throws(function () {
      parser.evaluate("a.b", { a: { b: 2 } });
    }, /member access is not permitted/);
  });
});
